...
f_jac = np.eye(9)
Q_co = np.zeros([6, 6])
Q_co[0:3,0:3] = var_imu_f * np.eye(3)
Q_co[3:6, 3:6] = var_imu_w * np.eye(3)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    C_ns = Quaternion(q_est[k-1][0], q_est[k-1][1], q_est[k-1][2], q_est[k-1][3]).to_mat()
    p_est[k] = p_est[k-1] + (delta_t * v_est[k-1]) + ((np.power(delta_t, 2)/2) * (np.dot(C_ns, imu_f.data[k-1] + g)))
     
    v_est[k] = v_est[k-1] + delta_t * (np.dot(C_ns, imu_f.data[k-1]) + g)

    imu_w.data[k-1] = imu_w.data[k-1]
    q_est[k] = Quaternion(axis_angle=(imu_w.data[k-1]* delta_t)).quat_mult_right(q_est[k-1])
    

    # 1.1 Linearize the motion model and compute Jacobians
    f_jac[0:3, 3:6] = delta_t * np.eye(3)
    f_jac[3:6, 6:] = -1* skew_symmetric(np.dot(C_ns, imu_f.data[k-1])) * delta_t
    Q_co[0:3,0:3] = var_imu_f * np.eye(3)
    Q_co[3:6, 3:6] = var_imu_w * np.eye(3)
    Q_co = np.power(delta_t, 2) * Q_co

    # 2. Propagate uncertainty

    p_cov[k] = np.dot(f_jac , np.dot(p_cov[k-1], np.transpose(f_jac))) + np.matmul(l_jac, np.matmul(Q_co, np.transpose(l_jac)))

    # 3. Check availability of GNSS and LIDAR measurements
    if gnss_i < gnss.t.shape[0] and gnss.t[gnss_i] == imu_f.t[k-1]:
        y_k = gnss.data[gnss_i] + var_gnss
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], y_k, p_est[k], v_est[k], q_est[k], R_gnss)
        gnss_i += 1

    if lidar_i < lidar.t.shape[0] and lidar.t[lidar_i] == imu_f.t[k-1]:
        y_k = lidar.data[lidar_i] + var_lidar
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov[k], y_k, p_est[k], v_est[k], q_est[k], R_lidar)
        lidar_i += 1
    # Update states (save)
